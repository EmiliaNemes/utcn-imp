...
{
  auto exit = MakeLabel();
  auto elsecond = MakeLabel();

  LowerExpr(scope, ifStmt.GetCond());
  EmitJumpFalse(elsecond);
...
// -----------------------------------------------------------------------------
// crestem stiva =>emitpop
// emitem opcode-ul
// emitem operand-ul

void Codegen::LowerIntExpr(const Scope &scope, const IntExpr &intgr)
{
  depth_ += 1;
  Emit<Opcode>(Opcode::PUSH_INT);
  Emit<int64_t>(intgr.GetVal());
}
